...
private:
	...
	template <class T>
	void find (long int& mid, const T& vec, const std::string& name);
public:
	...
	const Configuration& getConfig (const std::string& name);
private:
...
public:
	...
	long double getReal (const std::string& name);
	...
	long int getInt (const std::string& name);
