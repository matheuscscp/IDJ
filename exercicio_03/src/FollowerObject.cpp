#include "../include/FollowerObject.hpp"
...
FollowerObject::FollowerObject (
	...
	Sprite* sprite
) : GameObject ( r, depthconst ), sprite ( sprite )
{
...
{
	path.push ( r2vec (
		InputManager::instance ()->mouseDownX () + Camera::instance ()->r.x ( 0 ) * depthconst,
		InputManager::instance ()->mouseDownY () + Camera::instance ()->r.x ( 1 ) * depthconst
	) );
}
...
void FollowerObject::renderLines ()
{
	...
	{
		SDLBase::drawLine (
			dest.x ( 0 ) - Camera::instance ()->r.x ( 0 ) * depthconst,
			dest.x ( 1 ) - Camera::instance ()->r.x ( 1 ) * depthconst,
			r.x ( 0 ) - Camera::instance ()->r.x ( 0 ) * depthconst,
			r.x ( 1 ) - Camera::instance ()->r.x ( 1 ) * depthconst,
			0xFFFFFF,
			30
		);
		...
		{
			SDLBase::drawLine (
				path.front ().x ( 0 ) - Camera::instance ()->r.x ( 0 ) * depthconst,
				path.front ().x ( 1 ) - Camera::instance ()->r.x ( 1 ) * depthconst,
				vtmp.x ( 0 ) - Camera::instance ()->r.x ( 0 ) * depthconst,
				vtmp.x ( 1 ) - Camera::instance ()->r.x ( 1 ) * depthconst,
				0xFFFFFF,
				30
			);
