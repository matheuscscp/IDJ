...
void Button::render ()
{
	if ( ( tileset ) && ( active ) )
	{
		...
		x = r.x ( 0 ) - Camera::instance ()->r.x ( 0 ) * depthconst * use_camera;
		y = r.x ( 1 ) - Camera::instance ()->r.x ( 1 ) * depthconst * use_camera;
...
bool Button::mouseInside () const
{
	...
	rect.x = r.x ( 0 ) - Camera::instance ()->r.x ( 0 ) * depthconst * use_camera;
	rect.y = r.x ( 1 ) - Camera::instance ()->r.x ( 1 ) * depthconst * use_camera;
