...
public:
	...
	void set (const unsigned int& i, const unsigned int& j, const Scalar& param);
...
template <unsigned int m, unsigned int n>
LinearMap< SUBSIZE(m), SUBSIZE(n) > submatrix (	const LinearMap< m, n >& param,
...
template <unsigned int n>
struct RnLinearOp { typedef LinearMap< n, n > type; };
...
R2LinearOp r2lop (	const Scalar& a11, const Scalar& a12,
...
template <unsigned int m, unsigned int n>
const Scalar& LinearMap< m, n >::a (const unsigned int& i, const unsigned int& j) const
{
	return src_.a[i][j];
}
...
template <unsigned int m, unsigned int n>
Scalar LinearMap< m, n >::cofactor (const unsigned int& i, const unsigned int& j) const
{
	if ( m != n )
